...
function Header({
  ...
  //   Auditor: (item) => item.userType === 'Auditor',
  //   FieldAgent: (item) => item.userType === 'Field Agent'
  // };

  // Object.keys(userTypeFilters).forEach((userType) => {
  //   const filteredResponse = response.filter(userTypeFilters[userType]);

  //   if (filteredResponse.length > 0) {
  //     data.push({
  //       title: `${userType} Notifications (${filteredResponse.length})`,
  //       description: '',
  //       avatar: '',
  //       children: filteredResponse.map((item) => ({
  //         title: `${item.userName} registered through ${item.email}`,
  //         description: `on ${item.createdAt.substring(0, 10)}`
  //       }))
  //     });
  //   }
  ...
  // if (data.length === 0) {
  //   data.push({ title: "You have no new notifications!!!" });
  // }
